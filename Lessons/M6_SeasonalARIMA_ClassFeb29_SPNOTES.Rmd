...
Days <- as.data.frame(seq.Date(from=as.Date("2001/01/01"),to=as.Date("2022/11/01"), by ="month")) #by function has to specify month
colnames(Days) <- "Month"
#want to make it same as original
...
# then you can run ts.clean or na.approx to interpolate
```
...
#Lets difference the series once at lag 12 to remove the seasonal trend.n- doing a seasonal difference, diff at lag 12; and trend diff at lag 1
...
# unit root: when seeing on PACF that lag 1 is close to 1, may have a unit root, which means need to difference
...
```{r}
Model1 <- Arima(ts_electricity_price[,"Residential"], order =c(1,1,1),
                seasonal = c(0,1,1),
                include.drift = FALSE)
# don't need to re-specify seasonal lags because already had period in the time series
print(Model1)
...
#check to see AIC; it should be more negative to be better
```
...
#compare seasonal arima
# better to keep the seasonal data; make sure the model will attend 
# can create a model that can handle both
ARIMA_autofit <- auto.arima(deseasonal_residential_price,
                            max.D=0,
                            max.P = 0,
                            max.Q = 0)
print(ARIMA_autofit)
