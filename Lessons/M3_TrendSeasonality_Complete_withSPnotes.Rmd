...
```{r}
# methods: can remove the season, keep the trend, and run mann-kendall test

# another way: aggregate the data set; can consider the yearly average

iHP=1
#Group data in yearly steps instances
# setting the rows to be 12, which is for the months;
...
# score is positive so its an increasing; score is S
# seasonal mann kendall: looking at each month individually
# mann kendall, only looking at the yearly averages
...
# spearman correlation is 0.28; not very high but as expected

#with cor.test you can get test statistics
sp_rho=cor.test(inflow_data_yearly,my_year,method="spearman")
print(sp_rho)

# reject p value and null - the data has a trend
```
...
# reject the null for this test means there is no 
...
# conclude we have a trend stationary
```
