...
# input should always be a time series; x is always the original time series; then seasonal component and trend and random component
# does a moving average trend composition; usually going to be size of seasonal lag - centered moving average, if specified 12, 6 before and 6 after

# trying to capture the year average if that is changing over time

# decompose uses seasonal means model - can only fit constant seasonal patterns; if seasonal pattern is changing over time, won't remove it
# "additive": can choose additive or multiplicative; additive means sum trend, seasonal, random; depends on how it is used over time.
#how to decide which to try: try both and see which made random look most random
#when multiplicative: when seasonal component is a function of time; whenever waves are changing magnitude over time - multiplicative
#always start with additive model
#stl() function - does the same as decompose; but only does additive (seasonal trend linear decomposition)

iHP=1
decompose_inflow_data=decompose(ts_inflow_data[,iHP],"additive")
plot(decompose_inflow_data)

# now look for outliers in the random component
# differencing is different from this; differencing when the moving average/linear trend isn't removing the trend
...
# getting NA's
# when you decompose - more NA's; because using the moving average - going to lose some - have to start the series after the first 6 months

#Note random series has some missing values, that is why we got NAs

#Compute mean and standard deviation without missing values - remove the NA's at the beginning and end of the dataset; its the moving average 
...
#tsclean function: going to remove missing data/additive outliers from data - performs internally the function
# running the tsclean function: clean series; did identify outliers and removed them; removed and replaced by the mean
# do we want to replace all these outliers? need to understand where they are coming from?
```
...
#april is the transition to dry season - the other ones aren't outliers! don't remove

# following plot: showing - and + 3 sd; anything outsde teh 3 stdevs, is an outlier
...
# function outlier: value that is deviating the most from the mean
outlier(df_inflow_random$inflow) 
                  
#Function chisq.out.test check if extreme value is outlier
# runs on the most deviating value; very low p value - can say there is an outlier; running a hyp test to find if it is an outlier - looking at the test statistic
# risk adverse - conservative projection
grubbs_test <- grubbs.test(df_inflow_random$inflow)
print(grubbs_test) #look at the p-value to find the decision 

#If you need to remove outlier use rm.outlier() (doing what the tsclean function does; we are doing it manually; must specify what the fill is - replacing it with local mean)
...
# important because can adjust the p value sensitivity
# do outlier detection in random component only
