...
# see that the other cents for kwh, there are NA's, not worried about it
...
#notice it is decreasing; not right - need to process it; have to edit the date
...
# GG plot is fixing the date to be in the right order; 
TS_Plot <- 
...
plot(electricty_price_processed$Residential, type = "l")
...
```{r}
# when you convert to a time series, start on the one AFTER then date 
...
# make sure the first and last match the dataframe; there may be missing rows/data - always check the head and tails
# would be merging the two data sets - left merge by date; will create the missing rows but with the NA's - need to have continuous observations
...
# ACF: has a slow decay; still have significant coeff after 40 lags; looks like there is a trend there; can see the seasonality
# PACF at this point, doesn't tell us much at this point; but autoregressive, because strong at lag 1
...
# random looks random, centered at 0, can see the trend
# no mathmatical way to show if random is random
# next step, should remove the seasonality
```
...
#comparing deseasoned and regular ts
...
#Note seasonality is gone! but it is still a slow decay and the trend is still there
...
# there is a unit root, need to difference the series

#Lets difference the series to remove the trend.
#Difference the data at lag 1 and remove NA
deseasonal_residential_price_diff <- diff(deseasonal_residential_price,
                                          differences =1)

# cannot tell if it is stochastic or deterministic
...
print(Model_111) # take into account the negative sign
