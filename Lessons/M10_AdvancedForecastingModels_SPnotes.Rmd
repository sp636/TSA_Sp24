...
* ETS
* ARIMA with dynamic harmonic fourier components: based on sin and cosin functions
* TBATS
* Neural Network:like ARIMA, but allows for non-linear relationships between observations

SP notes: add seasonality as an external regressor. ex: different mean for each month of the year
...
# cannot upload to the github repository
...
# temp object created; read the temp file. have to unzip with unz().
#must unlink(temp) - removes the temp file
temp <- tempfile()
...
# idea of the pipe is to process the dataframe; provide the pipe with the initial data frame, leave the pipe with the processed df
act_power_all <- household_all %>% 
  mutate( Global_active_power = as.numeric(Global_active_power)) %>%  
  mutate( Date = dmy(Date)) %>% #day, month, year 
  mutate( Time = hms(Time)) %>%  #also from lubridate; h:m:s
  mutate( Year = year(Date), #create our own date column; separating because aggregating later
          Month = month(Date), 
          Day = day(Date), 
          Hour = hour(Time), 
          Min = minute(Time)) %>% 
...
# filter out the NA's for now; then group - start with the larger (year) then to hour
#summarise for active power column; for those 60 rows, average them together. the 60 rows will be replaced with just the mean, and that replaces the 60 rows
# group by hour; summarizes by the LAST column
...
# now leaving 'hour' out; R has to know what to do with the active power column
...
# filtering out the NA's because we are taking the average
...
* **seasonal.periods** vector with the seasonal periods (with all the seasonal frequency for each) 
...
# since it is starting at the first hour of the day, don't have to specify anything here

ts_act_power_daily <- msts(act_power_daily$daily_mean_active_power, 
                           seasonal.periods =c(7,365.25),
                           start=c(2006,12,16))

#once you have hourly and daily data, can copy and paste
```
...
# first plot: original, second: trend; others are seasonal
# seasonal 7: weekly; seasonal 365: yearly seasonal
#and remainder
ts_act_power_hourly %>% mstl() %>%
  autoplot()
# messy but can see some patterns
```
...
# subsetting by ending it early for the train
# for the test, subset to be the end of the dataset
# can subset a ts object
n_for = 365
...
# only using the train for model fitting

#Plot foresting results
autoplot(ETS_fit) + ylab("Active Power")
#dark lines are the means estimated
#we are combining the stl with exponential smoothing; there is no seasonal terms because represented with STL
# model looks good; has variability, maintaining the same behavior
...
# fit arima; seasonal=false -- fooling arima to say it isn't seasonal; and lambda. we are going to add external regressors

#function fourier(): finds better terms 
# to model teh weekly behavior, only need two terms - weekend and weekday
# what is th min number of harmonics is 12, max number of terms, might need less. yearly seasonal pattern. 
#auto.arima just gives fit; need to do the FORECAST too
...
# iwant 365 forecast for fourier, forecast inside the forecast
# needs to find the fit for the same you put int the auto arima
...
# modeling the parameters; adding the model of the standard deviation too
...
autoplot(NN_for) +
  ylab("Active Power") 
# doesn't plot the confidence interval
